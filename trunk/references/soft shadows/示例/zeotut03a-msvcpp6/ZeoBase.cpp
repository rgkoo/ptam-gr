/***************************************************************
*                                                              *
*            Overlords Revised OpenGL Basecode                 *
*                Based on NeHe's basecode                      *
*  Huge Thanks To Jeff Molofee's, Maxwell Sayles & Peter Puck  *
...
#include "ZeoBase.h"														// Header File For The overlord Basecode
...
static unsigned int timingFormula;										// chooses our timing formula
...
{
	DWORD windowStyle = WS_POPUPWINDOW;									// Define Our Window Style
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	window.init.title			= "ZeoBase GL Framework";			// Window Title
	...
	window.frameRate			= 0.016f;								// run at aprox 60fps
	window.x					= 0;									// x position of window
	window.y					= 0;									// y position of window
	timingFormula				= TIMER_FULLDT;					// select wich timing formula to use
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							switch(timingFormula)						//Choose timing formula
							{

							case TIMER_FULLDT:							// run at a variable speed with dynamic framerate	
									Update (window.deltaTime);			// Update The Counter
									Draw ();							// Draw Our Scene
									SwapBuffers (window.hDC);			// Swap Buffers (Double Buffering)
									break;

							case TIMER_SETDT:							// run at a fixed speed	 with dynamic framerate
									
									dtTemp+=window.deltaTime;			// Update dtTemp Counter with deltatime
								
									while (dtTemp>window.frameRate)		// Update the frame at a fixed framerate
									{
							...
							case TIMER_SETDT_PAUSED:					// same as above but add a sleep to reduce framerate and cpu/gpu utilization
								
									dtTemp+=window.deltaTime;			// Update dtTemp Counter with deltatime
								
									while (dtTemp>window.frameRate)
									{
										Update (window.frameRate);		// Update The Counter
										dtTemp-=window.frameRate;
									}
									Draw ();							// Draw Our Scene
									SwapBuffers (window.hDC);			// Swap Buffers (Double Buffering)
									...
									if (timeSpent<window.frameRate)		// we are running faster than the set frameratre
									{ Sleep((unsigned long)((window.frameRate-timeSpent)*900)); } //sleep for 90% of the remaining time
									break;


								case TIMER_FULLDT_PAUSED:				// run at a variable speed with dynamic framerate and cpu reduction
									Update (window.deltaTime);			// Update The Counter
									Draw ();							// Draw Our Scene
									SwapBuffers (window.hDC);			// Swap Buffers (Double Buffering)
