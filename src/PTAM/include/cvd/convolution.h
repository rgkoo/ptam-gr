...
template <class T> void convolveGaussian(const BasicImage<T>& I, BasicImage<T>& out, double sigma, double sigmas=3.0)
{
	...
	for (int j=0; j<ksize; j++) {
	    ...
	    for (int k=0; k<ksize; k++)
			hsum += (input[max(j-k-1,0)] + input[j+k+1]) * kernel[k];
	    next_row[j] = hsum;
	}
	// middle of row
	input += ksize;
	...
	for (int j=w-ksize; j<w; j++, input++) {
	    ...
	    const int room = w-j;
	    for (int k=0; k<ksize; k++) {
		hsum += (input[-k-1] + input[min(k+1,room-1)]) * kernel[k];
	    }
	    next_row[j] = hsum;
	}
	// vertical
	if (i >= swin) {
		...
		for (int r=0; r<ksize; r++) {
			...
			const sum_type* row2 = rows[min(ksize+r+k+2, swin)];
			add_multiple_of_sum(row1, row2, m, outbuf, w);
		    }
		    cast_copy(outbuf, output, w);
		    output += w;
		...
		for (int k=0; k<ksize; k++) {
		    const sum_comp_type m = kernel[k];
		    const sum_type* row1 = rows[max(r-k-1,0)+1];
