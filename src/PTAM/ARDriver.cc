...
ARDriver::ARDriver(const ATANCamera &cam, ImageRef irFrameSize, GLWindow2 &glw, ARGame &game)
  :mCamera(cam), mGLWindow(glw), mGame(game)
{
...
void ARDriver::Render(Image<Rgb<byte> > &imFrame, SE3 se3CfromW)
{
  ...
  glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,mnFrameBuffer);
	  ...
	  Matrix<4>& UFBLinearFrustumMatrix = mCamera.MakeUFBLinearFrustumMatrix(0.005, 100);
	  ...
	  mGame.DrawStuff(UFBLinearFrustumMatrix, se3CfromW);
